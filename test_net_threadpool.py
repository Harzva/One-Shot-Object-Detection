...
def bboxreg(rois, cls_prob, bbox_pred,index,det_tic,data  ):
...
if __name__ == '__main__':
  ...
  output_dir_vu = get_output_dir(imdb_vu, 'faster_rcnn_unseen_'+os.path.basename(load_name)[:-4])
  ...
  if args.group != 0:
    det_file = os.path.join(output_dir_vu, 'sess%d_g%d_seen%d.pkl'%(args.checksession, args.group, args.seen))
  else:
    det_file = os.path.join(output_dir_vu, 'sess%d_seen%d.pkl'%(args.checksession, args.seen))
  print(det_file)
  ...
  def say(iss):###因为
    if args.cuda:
      ...
      query   = Variable(torch.FloatTensor(1).cuda())
      gt_boxes = Variable(torch.FloatTensor(1).cuda())
    index,data=iss[0],iss[1]
    ...
    all_boxes=bboxreg(rois, cls_prob, bbox_pred,index,det_tic,data)
    return all_boxes
  ...
  temp=list(zip(ratio_index_vu[0],data_iter_vu))
  pbar = tqdm(total=len(ratio_index_vu[0]), ncols=50)
  pbar.set_description(f"{args.dataset}_test")
  # for index,data in enumerate(dataloader_vu):
  #   say(index,data)
  for _ in executor.map(say,temp):
    pass
