...
def read_weight():
    import numpy as np
    data = np.zeros((500,201),dtype=np.float32)
    with open('word_w2v.txt') as f:
...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        self.word_embs.weight.data.copy_(torch.from_numpy(read_weight()))
    ...
    def forward(self, im_data, query, im_info, gt_boxes, num_boxes, query_word_idx):
        ...
        # ==========
        # 更新语义向量后再和视觉向量级联
        new_query_word_vector = self.trans(query_word_vector)
        query_feat = torch.cat((query_feat,new_query_word_vector),dim=1)
        query_feat = self.trans1(query_feat)
        # ==============
