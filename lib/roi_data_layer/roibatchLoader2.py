...
class roibatchLoader(data.Dataset):
  ...
  seen=True, class_to_name = None,word_name_to_index = None):
    self._roidb = roidb
    self._query = query
    self._num_classes = num_classes
    self.class_to_name = class_to_name
    if self.class_to_name != None:
        tmp_dict = {}
        for key, value in self.class_to_name.items():
            print(key)
            tmp_dict[value] = key.strip()
        self.class_to_name = tmp_dict
    self.word_name_to_index = word_name_to_index
    #200 imagenet{'accordion': 0, 'airplane': 1, 'ant': 2, 'antelope': 3, 'apple': 4, 'armadillo': 5, 'artichoke': 6, 'axe': 7, 'baby bed': 8, 'backpack': 9, 'bagel': 10, 'balance beam': 11, 'banana': 12, 'band aid': 13, ...}
    ...
    # self.word_vector = word_vectors[0]
    ...
    self._cat_ids = [
        ...
        ] if self._num_classes ==81 else list(range(1,21))
    self._classes = {
    ...
    # self.filter(seen)
    self.list_ind =pascal_voc('trainval', '2007').filter(seen) if self._num_classes==21 else  self.filter(seen)
    print('your split is ','*'*80,{i:self.class_to_name[i] for i in self.list_ind})
    self.probability()
  def __getitem__(self, index):
    ...
    if self.training:
        ...
        #print("self._classes_:",self._classes)
        ...
        src_name = self.class_to_name[choice]
...
#   
  def filter(self, seen):
    if seen==1:
      self.list = cfg.train_categories#11
