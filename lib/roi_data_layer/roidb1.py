...
def prepare_roidb(imdb):
  ...
  for i in range(len(imdb.image_index)):
    ...
    #print("roidb[i]['max_classes']",i,roidb[i]['max_classes'])
...
def filter_roidb(roidb):
    ...
    while i < len(roidb):
      if len(roidb[i]['boxes']) == 0:
        ...
        '''
              # ALISURE
      elif keey_class is not None:
        is_keep = [one for one in roidb[i]["gt_classes"] if one in keey_class]
        if len(is_keep) == 0:
          del roidb[i]
          i -= 1
          pass
...
def combined_roidb(imdb_names, training=True, seen=1):
  ...
  def get_roidb(imdb_name, training):
    ...
    class_to_name = imdb._class_to_ind
    print("class_to_name:",class_to_name)



    return imdb, roidb, imdb.cat_data, imdb.inverse_list,class_to_name

  imdbs = []
  roidbs = []
  querys = []
  for s in imdb_names.split('+'):
    imdb, roidb, query, reserved,class_to_name = get_roidb(s, training)
    imdbs.append(imdb)
    roidbs.append(roidb)
    querys.append(query)
  imdb = imdbs[0]
  ...
  if len(roidbs) > 1 and training:
    ...
    imdb._class_to_ind=class_to_name
  if training:
  ...
  return imdb, roidb, ratio_list, ratio_index, query,imdb._class_to_ind