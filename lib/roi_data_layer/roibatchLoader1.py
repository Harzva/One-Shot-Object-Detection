...
class roibatchLoader(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if self.training:
        ...
        #
        #padding_data, query, im_info, gt_boxes_padding, num_boxes
        ...
        # ===================
        # 查询/支持目标互换具体细节----一对一互换
        if np.random.random() < 0.5 and len(target_list) > 0:
        ...
        # =========================
            #####
            # src = cv2.cvtColor(padding_data.numpy().copy().transpose(1,2,0),cv2.COLOR_RGB2BGR)

            # ca= cv2.cvtColor(crop_area.numpy().transpose(1,2,0),cv2.COLOR_RGB2BGR)
            # q = cv2.cvtColor(query.numpy().transpose(1,2,0),cv2.COLOR_RGB2BGR)
            ...
            # t1 = cv2.cvtColor(padding_data.numpy().copy().transpose(1,2,0),cv2.COLOR_RGB2BGR)
