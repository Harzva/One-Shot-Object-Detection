...
if __name__ == '__main__':
    ...
    if args.use_tfboard:
        ...
        logger_summary = SummaryWriter(log_dir=f"logs/{os.path.basename(output_dir)}")
    logger =Tools.get_logger(os.path.basename(output_dir), log_dir=f"logs/{os.path.basename(output_dir)}/{os.path.basename(output_dir)}.log")
    # print('Using config:')
    # pprint.pprint(cfg)
    ...
    cfg['CUDA']=True
    logger.info('*'*60,'Using config:')
    pprint.pprint(cfg)

    # torch.backends.cudnn.benchmark = True
    # torch.backends.cudnn.deterministic = True
    if torch.cuda.is_available() and not args.cuda:
        logger.info("*"*60,"WARNING: You have a CUDA device, so you should probably run with --cuda")
    ...
    test_imdb, test_roidb, test_ratio_list, test_ratio_index, test_query, test_class_to_name = combined_roidb(args.imdb_name, True, seen=2)


    def demo(args,seen):
        imdb, roidb, ratio_list, ratio_index, query, class_to_name = combined_roidb(args.imdb_name, True, seen)
        ...
        return dataloader,class_to_name,imdb,train_size
    train_dataloader,class_to_name,imdb,train_size=demo(args,seen=1)

    test_dataloader,class_to_name,imdb,test_size=demo(args,seen=2)
        # ==========
    ...
    name_list = []
    index_list = []
    logger.info("class_to_name:", class_to_name)
